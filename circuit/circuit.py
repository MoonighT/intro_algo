...
class PriorityQueue:
    ...
    def min_heapify_up(self, index):
        #if node break heap property, go up
        if index > self.root:
            ...
            if self.heap[index] < self.heap[parent]:
                ...
                self.min_heapify_up(self.parent(index))

    def min_heapify_down(self, index):
        left_index = self.left_child(index)
        right_index = self.right_child(index)
        smaller_index = index
        smaller = self.heap[index]
        if left_index < len(self.heap):
            if self.heap[left_index] < smaller:
                smaller_index = left_index
                smaller = self.heap[left_index]

        if right_index < len(self.heap):
            if self.heap[right_index] < smaller:
                smaller_index = right_index
                smaller = self.heap[right_index]

        if smaller_index != index:
            self.heap[smaller_index] = self.heap[index]
            self.heap[index] = smaller
            self.min_heapify_down(smaller_index)
    ...
    def pop(self):
        ...
        self.heap[self.root] = self.heap[len(self.heap)-1]
        
   
        self.heap = self.heap[:len(self.heap)-1]
        if len(self.heap) > 0:
            self.min_heapify_down(self.root)
        return min_value
